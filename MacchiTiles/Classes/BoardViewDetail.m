...
@implementation BoardViewDetail
...
- (void)initIconImages
{
	...
	{
		NSString *tileFilesNO = [[NSString alloc] initWithFormat:@"%d%@_NO.png", i+1, symbol];
		[tileIconsNormal addObject:[UIImage imageNamed:tileFilesNO]];
		
		NSString *tileFilesNE = [[NSString alloc] initWithFormat:@"%d%@_NE.png", i+1, symbol];
...
{	
	...
	for (int i = 0; i < MAX_ROWS; i++)
	{
		...
		{
			int stringNum = [[self.currentGame objectAtIndex:((i*8)+j)] intValue]; // get each tile value from game file
			NSInteger indexNum = (NSInteger)stringNum - 1;						   // change index to 0-based
			
//			CGRect tileRect = CGRectMake(j*40.0, (i*38.0)+2.0, 40.0, 38.0);
			CGRect tileRect = CGRectMake(j*TILE_W, (i*TILE_H) + H_BUFFER, TILE_W, TILE_H);
			// setup tile views
			...
			[tempPath setImage:[UIImage imageNamed:@"tilepath2.png"]];
...
- (void)saveViewsForUndo
{
	...
	if ([self.view viewWithTag:UNDO_FIRST] != nil)
		[self clearSelectedView:(UIImageView *)[self.view viewWithTag:UNDO_FIRST]];
	if ([self.view viewWithTag:UNDO_SECOND] != nil)
		[self clearSelectedView:(UIImageView *)[self.view viewWithTag:UNDO_SECOND]];
	// save views 1 and 2 for undo
	[[self.view viewWithTag:1] setTag:UNDO_FIRST];
	[[self.view viewWithTag:2] setTag:UNDO_SECOND];
...
- (void)undoViews
{
	...
	[[self.view viewWithTag:UNDO_FIRST] setHidden:NO];
	[[self.view viewWithTag:UNDO_SECOND] setHidden:NO];
...
-(int)getIndexFromPoint:(CGPoint)location asRow:(BOOL)row
{
	...
	if(row) {
		if (location.y < 458.0)
			indx = (int)((location.y-H_BUFFER) / TILE_H);
		else
			indx = MAX_ROWS-1;
	}
	else
		indx = (int)(location.x / TILE_W);
...
-(CGPoint)closestGridCenterTo:(CGPoint)location
{
	...
	CGFloat xCoord = (TILE_W/2)+(TILE_W* col);
	CGFloat yCoord = (TILE_H/2)+(TILE_H* row);
	CGPoint retval = CGPointMake(xCoord, yCoord+H_BUFFER);
...
- (CGPoint)getCenterFromIndex:(NSInteger)index
{
	...
	if (index >= 0 && index < MAX_TILES)
	{
		...
		int row = (int)(index / MAX_COLS);
		
		CGFloat xCoord = (TILE_W/2)+(TILE_W* col);
		CGFloat yCoord = (TILE_H/2)+(TILE_H* row);
		retval.x = xCoord;
		retval.y = yCoord + H_BUFFER;
	}
	return retval;
}
...
{
	NSInteger retval = MT_ERROR;
	
	if (bound == WEST){       // check west 
		if ((index % MAX_COLS) != 0)
			retval = index - 1;
		else
			retval = -1;      // no west tile
	}
	else if (bound == NORTH){ // check north
		if ((index - MAX_COLS) >= 0)
			retval = index - MAX_COLS;
		else
			retval = -2;      // no north tile
	}
	else if (bound == EAST){  // check east
		if ((index % MAX_COLS) != (MAX_COLS-1))
			retval = index + 1;
		else
			retval = -4;      // no east tile
	}
	else if (bound == SOUTH){ // check south
		if ((index + MAX_COLS) < MAX_TILES)
			retval = index + MAX_COLS;
		else
...
- (void)movableRoutesFrom:(NSInteger)index
{
	...
	// check west bound
	NSInteger recur = [self getAdjacencyIndexAt:index ofSide:WEST];
	while (recur >= 0){
		if ([self.boardMap objectAtIndex:recur] == @"Y"){
			[self.horizMoveVec addObject:[NSNumber numberWithInt:recur]];
			recur = [self getAdjacencyIndexAt:recur ofSide:WEST];
		}
		else
			break;
	}
	// check north bound
	recur = [self getAdjacencyIndexAt:index ofSide:NORTH];
	while (recur >= 0){
		if ([self.boardMap objectAtIndex:recur] == @"Y"){
			[self.vertiMoveVec addObject:[NSNumber numberWithInt:recur]];
			recur = [self getAdjacencyIndexAt:recur ofSide:NORTH];
		}
		else
			break;
	}
	// check east bound
	recur = [self getAdjacencyIndexAt:index ofSide:EAST];
	while (recur >= 0){
		if ([self.boardMap objectAtIndex:recur] == @"Y"){
			[self.horizMoveVec addObject:[NSNumber numberWithInt:recur]];
			recur = [self getAdjacencyIndexAt:recur ofSide:EAST];
		}
		else
			break;
	}
	// check south bound
	recur = [self getAdjacencyIndexAt:index ofSide:SOUTH];
	while (recur >= 0){
		if ([self.boardMap objectAtIndex:recur] == @"Y"){
			[self.vertiMoveVec addObject:[NSNumber numberWithInt:recur]];
			recur = [self getAdjacencyIndexAt:recur ofSide:SOUTH];
		}
		else
			break;
	}
...
- (void)touchesMoved:(NSSet *)touches withEvent:(UIEvent *)event {
	...
	[self showGuideViewAt:touchCurrentPt];
		...
		if (deltaX >= kMinimumGestureLength && deltaY <= kMaximumVariance && !hasMovedVerti && !hasTileMovedOutOfBounds) {
...
//			[[self.view viewWithTag:tagView] setCenter:CGPointMake(xMoved, yMoved)];
			...
			if ((touchCurrentPt.x - kMaximumVariance) > maxMovement ||
				(touchCurrentPt.x + kMaximumVariance) < minMovement)
				hasTileMovedOutOfBounds = YES;
		}
		// Vertical swipe detected
		else if (deltaY >= kMinimumGestureLength && deltaX <= kMaximumVariance && !hasMovedHoriz && !hasTileMovedOutOfBounds) {
...
//			[[self.view viewWithTag:tagView] setCenter:CGPointMake(xMoved, yMoved)];
			...
			// check to see if swipe is out of bounds
			if ((touchCurrentPt.y - kMaximumVariance) > maxMovement ||
				(touchCurrentPt.y + kMaximumVariance) < minMovement)
				hasTileMovedOutOfBounds = YES;
		}
		else if ((deltaX >= kMinimumGestureLength && deltaY > kMaximumVariance) ||
			     (deltaY >= kMinimumGestureLength && deltaX > kMaximumVariance) || hasTileMovedOutOfBounds) { 
...
- (void)touchesEnded:(NSSet *)touches withEvent:(UIEvent *)event {
	// turn off guide view right away
	[self.guideView setHidden:YES];
